...
const svg = d3.select("#map_score_legislature").append("svg")
    .attr("width", width)
    .attr("height", height);
...
const tooltip = d3.select(".tooltip_score_legislature")
    .style("opacity", 0);

// Define a color scale
const color = d3.scaleQuantize()
    .domain([0, 10]) // Assuming your CSV values range from 0 to 8
...
//console.log('No data found for', data);
        ...
        const dataMap = new Map(data.map(row => {
            ...
            return [row.fips, {
                ...
                FemaleRepresentationScore: row.FemaleRepresentationScore
            }];
        }));
                ...
                try {
                    if (d && d.properties && 'FIPS' in d.properties) {
                       ...
                       // console.log('No data found for', dataMap);
                        ...
                        return color(dataRow.FemaleRepresentationScore);
                    } else {
                    ...
                    if (!d.properties || !countyData) {
                        ...
                        tooltip.html(
                            `County: ${d.properties.NAME}<br/>
                             Representation Score: ${countyData.FemaleRepresentationScore}`
                        )
                    }
...
// Create a legend
    const legend = svg.append("g")
        .attr("class", "legend")
        .attr("id", "map-legend_score_legislature") // Unique ID for the legend
